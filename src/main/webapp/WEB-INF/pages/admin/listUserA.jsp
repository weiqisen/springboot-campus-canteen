...
<section class="Hui-article-box">
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> <a onclick="location='index'">首页</a>
        ...
        用户管理
        <span class="c-gray en">&gt;</span>
        普通用户 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
            ...
            <table class="table table-border table-bordered table-bg" id="mytable">
                ...
                <c:forEach items="${users}" var="users" varStatus="index">
                    <tr class="text-c">
                        ...
                        <td>${users.phone}</td>
                        <td>${users.sex}</td>
                        <td><c:if test="${users.role==1}">学生</c:if><c:if test="${users.role==2}">食堂员工</c:if>
                            <c:if test="${users.role==3}">管理员</c:if><c:if test="${users.role==4}">后勤员工</c:if></td>
